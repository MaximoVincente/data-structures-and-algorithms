...
const screenForNames = (arr) => {
  return /(?:Mr.\sMrs.\sMs.\sDr.)/.test(arr);
};
...
const toTitleCase = (arr) => {
  const newArr = [];
  arr.forEach((v) => {
    newArr.push(v.charAt(0).toUpperCase() + v.slice(1));
  });
  return newArr;
};
...
let biggerThanLuke = (arr) => {
  return arr.filter((i,v) => parseInt(v.mass) > 77 ? v : i).name + ' - ';
};
...
H
ere is an example of the input:
[
